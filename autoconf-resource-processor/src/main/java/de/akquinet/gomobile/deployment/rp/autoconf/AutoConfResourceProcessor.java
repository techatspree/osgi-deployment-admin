...
public class AutoConfResourceProcessor implements ResourceProcessor {
    ...
    private volatile LogService m_logService;				// injected by iPOJO
    private volatile ConfigurationAdmin m_configAdmin;	    // injected by iPOJO
    private volatile MetaTypeService m_metatypeService;		// injected by iPOJO
    ...
    public AutoConfResourceProcessor(BundleContext bc) {
        m_context = bc;
    }

    public void start() throws IOException {
        File root = m_context.getDataFile("");
        if (root == null) {
    ...
    public void dropAllResources() throws ResourceProcessorException {
        ...
        File basedir = m_context.getDataFile("");
        ...
        for (Iterator i = keys.iterator(); i.hasNext();) {
            ...
            for (Iterator j = configs.iterator(); j.hasNext();) {
                ...
                try {
                    ...
                    m_logService.log(LogService.LOG_ERROR, "Unable to roll back resource '" + name + "', reason: " + e.getMessage() + ", caused by: " + e.getCause().getMessage());
                }
                break;
            }
        }
    ...
    private ObjectClassDefinition getMetaTypeOCD(MetaData data, Designate designate) throws ResourceProcessorException {
        ...
        if (bundle != null) {
            MetaTypeInformation mti = m_metatypeService.getMetaTypeInformation(bundle);
            if (mti != null) {
