...
public class ClassifierTest {
    ...
    File testDir = new File("src/test/resources/classifier");
    ...
    public void setUp() throws VerificationException, IOException {
        ...
        verifier.deleteArtifact( Helper.TEST_GROUP_ID, "test-classifier", Helper.TEST_VERSION, "dp" );
    ...
    public void testPackage() throws IOException, VerificationException {
        ...
        // Check existency
        File result = new File(testDir + "/target/test-classifier-0.0.1-SNAPSHOT.dp"); // Expected name
        System.out.println("result : " + result.getAbsolutePath());
        Assert.assertTrue(result.exists());

        JarFile jar = new JarFile(result);
        JarEntry je = jar.getJarEntry("org.apache.felix.webconsole-3.1.0-bare.jar");
        ...
        Assert.assertEquals("de.akquinet.gomobile.deployment.test.classifier", sn);
        Assert.assertEquals("0.0.1.SNAPSHOT", ve);

        Attributes section = manifest.getAttributes("org.apache.felix.webconsole-3.1.0-bare.jar");
        String bsn = section.getValue("Bundle-SymbolicName");
        String bve = section.getValue("Bundle-Version");

        Assert.assertEquals("org.apache.felix.webconsole.bare", bsn);
        Assert.assertEquals("3.1.0", bve);
