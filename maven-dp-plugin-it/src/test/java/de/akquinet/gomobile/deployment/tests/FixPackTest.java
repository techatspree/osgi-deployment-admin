...
public class FixPackTest {


    private static final String TEST_NAME = "fixpack";
    ...
    public void testPackage() throws IOException, VerificationException {
        ...
        Assert.assertNull(je); // Missing set to true.
        ...
        String fp = manifest.getMainAttributes().getValue(Constants.DEPLOYMENTPACKAGE_FIXPACK);

        Assert.assertEquals("de.akquinet.gomobile.deployment.test." + TEST_NAME, sn);
        Assert.assertEquals("0.0.1.SNAPSHOT", ve);
        Assert.assertEquals("[1.0.0, 2.0.0)", fp);
        ...
        String missing = section.getValue(Constants.DEPLOYMENTPACKAGE_MISSING);

        Assert.assertEquals("org.apache.felix.bundlerepository", bsn);
        Assert.assertEquals("1.4.2", bve);
        Assert.assertEquals("true", missing);
